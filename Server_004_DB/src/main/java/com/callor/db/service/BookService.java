...
/*
 * 업무적인 분석이 없이 만드는 DB 프로젝트에서는 필수적으로 필요한 method
 * 
 * DB와 연동되는 프로젝트는 1개의 table마다 서비스가 만들어지는데 CRUD를 최소한 구현해야 한다
 * 
 */
public interface BookService {
	 ...
	 * Controller에서 데이터를 받고		-		Service 클래스에 데이터를 전달하여
	 * DB에 insert를 수행한다.
	 */
	 ...
	 * PK 칼럼에 조건을 1개만 부여하여 select 한 후 데이터를 return하는 method
	 */
	public BookVO findById(String bk_isbn);
	
	// 도서명으로 검색
	// 도서명을 전받아서 해당도서명의 모든 데이터를 return
	...
	/*
	 * 변경하고자 하는 데이터를 전달받아서 (전달받은 데이터에는 반드시 PK값이 포함되어야 한다.)
	 * PK로 조건을 설정하고 데이터를 UPDATE
	 */
	public void update(BookVO bookVO);	// UPDATE,	데이터 변경(갱신, 수정)
	
	// 삭제하고자 하는 데이터를 PK값을 매개변수로 전달받고 1개의 데이터를 삭제한다
