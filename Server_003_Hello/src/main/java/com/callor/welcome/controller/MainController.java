...
// 그런데 Network과 관련된 코드는 너무 복잡하고 다양한 기술을 요구하기 때문에 코드를 직접 작성하는 것은 너무 어렵다
// java에서 제공하는 JDK를 상속받아서 대부분의 기능을 처리하고 꼭 필요한 기능만 구현을 하게 된다.

// WebServlet Annotation을 가지고 있는 클래스는 Tomcat에 의해서 관리되고 사용자의 Request에 따라
// Tomcat이 method를 호출하고 기능을 수행한다
...
// 3. 프로젝트 있으면 /home 문자열을 다시 분석하여 프로젝트내의 클래스 중에서
// 	@WebServlet("/home") Annotation으로 설정된 클래스를 찾는다
// 4. 클래스의 doGet(...) method를 호출한다.
...
//		out.close();
		...
		PrintWriter out = resp.getWriter();
		out.println("Welcome Main Controller");
		out.print("<a href='");
