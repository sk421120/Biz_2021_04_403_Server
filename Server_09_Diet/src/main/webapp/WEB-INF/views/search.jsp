...
<style>
	...
	/*
	 ...
	 이러한 현산은 UI 화면을 눈에 거슬리게 하는 결과가 된다
	 
	 padding 주었을때 좌, 우의 padding을 밖으로 보내지 않고
	 내부로 흡수하는 옵션
	 
	 이 설정을 style의 맨 상단에 설정하면 padding으로 인한
	...
	* {
	...
	/*
		table 의 어떤 row에 마우스가 올라가면 바탕색을 #aaa로
		마우스 커서 모양을 손모양으로
	 */
	
	/*
		#aaa = AA AA AA = 10*10 10*10 10*10
		#000 ~ #fff gray level
		rgb(100,100,100)
		rgb(255,255,255) 화이트
...
<body>
	...
	form에 action을 임의로 지정하지 않으면 action="/form/search"가 자동으로 지정된다
	 ...
	 <form method="POST">
	 	...
	 	<input name="fd_name" placeholder="식품명을 입력한 후 Enter!">
	 	</p>
	 </form>
	 		...
	 		<c:when test="${not empty FOODS }">
	 			<table>
	 				...
	 				<%-- 
	 					...
	 					for 반복문으로 반복하면서 item을 getter하여
	 					FOOD(DTO type)에 담아라
	 				--%>
