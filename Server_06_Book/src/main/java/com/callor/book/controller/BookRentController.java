	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		if(subPath.equals("/seq")) {
			...
			if(strSeq == null || strSeq.equals("")) {
				...
				/*
				 * ServletContext app
				 * Tomcat을 기반으로 작성된 Web APP Service에서 요청(Req) 응답(Resp)를
				 * 총괄하는 정보를 담긴 객체
				 * 
				 * Web App Service를 구현하기 위하여 Req, Resp를 처리하는
				 * 여러가지 기능을 구현해야 하는데 그러한 기능을 미리 구현해 놓았기 때문에
				 * ServletContext를 getter 하는 것만으로 충분하다
				 * 
				 * DB 등으로 부터 조회된 데이터를 Web 에게 응답하고자 할 때 쉬운 방법으로
				 * 전달할 수 있도록 하는 기능이 미리 구현되어 있다
				 */
				ServletContext app = this.getServletContext();
			
				// bService가 return한 brDTO를 app객체에 BOOK이라는 속성변수로 세팅하기
				// 	app객체에 BOOK이라는 객체변수를 생성하고
				// 	BOOK 변수에 brDTO 값을 저장한다		Object BOOK = brDTO 형식의 코드
				...
				// webapp/WEB-INF/views/book.jsp 파일을 읽어서 Java 코드로 변환, 실행준비
			...
			// rent/order 로 요청하면 주문서작성 처음화면 보여주기
			// 회원이름을 입력하는 화면을 보여주기
...
//			app.setAttribute("BUYER", buyerDTO);
			...
			// 대여일자값을 생성하기 위하여 날짜클래스와 날짜포맷클래스를 사용하여
			// 대여일자 문자열 만들기
			...
			// INSERT 를 수행하기 위해 VO를 만들고 web에서 전달받은 도서ISBN과 회원 CODE를 Setting
