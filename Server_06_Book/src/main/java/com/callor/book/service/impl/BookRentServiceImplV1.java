...
public class BookRentServiceImplV1 implements BookRentService {
	...
	protected List<BookRentDTO> select(PreparedStatement pStr) throws SQLException {
		...
		while(rStr.next()) {
			...
			brDTO.setBr_tel(rStr.getString("연락처"));
	...
	public BookRentDTO findById(Long seq) {
		 ...
		 * 만약 매개변수를 직접 SQL 명령문을 다음과 같이 만들면
		 * " WHERE 도서명 = " + title;
...
//			rSet.close();
	...
	@Override
	public List<BookRentDTO> findByBookTitle(String title) {
		// TODO 도서명으로 조회
		String sql = " SELECT * FROM view_도서대여정보 ";
		sql += " WHERE 도서명 LIKE '%' || ? || '%' ";
	...
	public int insert(BookRentVO bookRentVO) {
		...
		try {
			...
			// insert, update, delete 와 관련된 SQL은 excuteUpate() method로 처리한다
			// 정상적으로 SQL이 성공하면 result 에는 0보다 큰 값이 담긴다
