...
public class HomeController {
	...
	/*
	 * Controller에서 HomeService 인터페이스를 상속받은 MyServiceImplV1 클래스를
	 * 사용하여 findById() method를 호출한 후 VO 데이터를 받고 싶다
	 * 
	 * 전통적인 Java 클래스간의 연결은 인터페이스로 객체를 선언하고
	 * 	클래스로 객체를 생성(초기화)하여 가지고 있다가
	 * 	필요한 mehtod를 호출할 수 있도록 만들어진다
	 * 
	 * 하지만 Spring Framework 환경에서는 객체를 미리 Spring이 
	 * 	만들어 보관하고 있다가 필요한 곳이 있으면 그때그때 배분하여
	 * 	사용할 수 있도록 만들어 준다
	 * 
	 * 프로젝트가 커져서 많은 객체가 필요할때 전통적 프로그래밍은
	 * 	모든 객체를 선언 생서앟여 가지고(메모리에 보관)있어야 하기 때문에
